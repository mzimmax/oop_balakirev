class A:
    ...
    С ней связан второй элемент вектора x со значением 0, следовательно,
    мы переходим по правой ветви и попадаем в вершину "будет кодером".
...
class TreeObj:
    def __init__(self, indx, value=None):
        ...
        value - значение, хранящееся в вершине (принимает значение None для вершин,
        у которых есть потомки - промежуточных вершин).
        При этом, в каждом создаваемом объекте класса TreeObj
        должны автоматически появляться следующие локальные атрибуты:
    ...
    @left.setter
    def left(self, next_obj):
        self.__left = next_obj
    ...
    @right.setter
    def right(self, next_obj):
        self.__right = next_obj
...
v_11 = DecisionTree.add_obj(TreeObj(1), root)
v_12 = DecisionTree.add_obj(TreeObj(2), root, False)
v_21 = DecisionTree.add_obj(TreeObj(3), v_11)
v_22 = DecisionTree.add_obj(TreeObj(4), v_11, False)
v_22 = DecisionTree.add_obj(TreeObj(4), v_12)
v_23 = DecisionTree.add_obj(TreeObj(5), v_12, False)
DecisionTree.add_obj(TreeObj(-1, "A"), v_21)
DecisionTree.add_obj(TreeObj(-1, "B"), v_21, False)
DecisionTree.add_obj(TreeObj(-1, "C"), v_22)
DecisionTree.add_obj(TreeObj(-1, "D"), v_22, False)
DecisionTree.add_obj(TreeObj(-1, "E"), v_23)
DecisionTree.add_obj(TreeObj(-1, "F"), v_23, False)

x = [1, 1, 0, 1, 0, 0, 0]
res = DecisionTree.predict(root, x)  # A
print(res)
x = [0, 0, 1, 0, 0, 0, 0]
res = DecisionTree.predict(root, x)  # D
print(res)